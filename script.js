document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateCornerHitsLabel() {
    const cornerHitsLabel = document.getElementById("cornerHits");
    cornerHitsLabel.textContent = `Corner Hits: ${cornerHits}`;
  }
  ...
  function animate() {
    ...
    if (x <= 0 && y <= 0) {
      cornerHits++;
      updateCornerHitsLabel();
    } else if (x <= 0 && y >= canvas.height - logoHeight) {
      cornerHits++;
      updateCornerHitsLabel();
    } else if (x >= canvas.width - logoWidth && y <= 0) {
      cornerHits++;
      updateCornerHitsLabel();
    } else if (x >= canvas.width - logoWidth && y >= canvas.height - logoHeight) {
      cornerHits++;
      updateCornerHitsLabel();
    }
